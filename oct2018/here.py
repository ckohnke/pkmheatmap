...
def calcCarTime(APP_ID,APP_CODE,lat1,lon1,lat2,lon2,tunit='s'):
    ...
    data = '?app_id=%s&app_code=%s&waypoint0=geo!%s,%s&waypoint1=geo!%s,%s&routeattributes=wp,sm,sh,sc&mode=fastest;car'%(APP_ID,APP_CODE,lat1,lon1,lat2,lon2)
...
if __name__ == "__main__":
    ...
    APP_ID = config[0]
    APP_CODE = config[1]    
    miles, time = calcCarTime(APP_ID,APP_CODE,39.755543,-105.221100,40.585258,-105.084419,'h') # Golden to FOCO - approx 72 miles, 1.2 hours
    print(miles,time)
